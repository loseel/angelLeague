            ...
            <div class="login-box-body">
                <form id="loginForm" @submit.prevent="sendPost">  
                    <div class="form-group has-feedback">
                        <input type="text" v-model="userID" name="userID" class="form-control input-lg" placeholder="Email">
                    </div>
                    <div class="form-group has-feedback last">
                        <input type="password" v-model="userPW" name="userPW" class="form-control input-lg" placeholder="Password">
                    </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const sendPost = async () => {
        console.log(userPW);
        const res = await axios.post('https://api.angelleague.io/v1/token', {
            clientSecret: userPW,
        })
...
.login-box .login-box-body form, .login-box .login-box-body .form-group:not(.last) {
    margin-bottom: 30px;
}
